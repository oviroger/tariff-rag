#!/usr/bin/env pwsh
# Test script para verificar mejora de preguntas estratégicas en vehículos

$API_URL = "http://localhost:8000/classify"
$timestamp = Get-Date -Format "yyyyMMdd_HHmmss"
$output_dir = "d:\MAESTRIA - copia\tariff-rag\test_results_$timestamp"

New-Item -ItemType Directory -Path $output_dir -Force | Out-Null

Write-Host "=== PRUEBA DE MEJORA: Preguntas Estratégicas en Vehículos ===" -ForegroundColor Green
Write-Host "Timestamp: $timestamp" -ForegroundColor Cyan
Write-Host "Output Directory: $output_dir" -ForegroundColor Cyan
Write-Host ""

# TURNO 1: Query inicial sobre autobús
Write-Host "📍 TURNO 1: Query inicial" -ForegroundColor Yellow
$turno1_query = "Necesito clasificar un autobús que voy a importar"
...
Write-Host "✓ TURNO 1 guardado en: $turno1_file" -ForegroundColor Green
...
Write-Host "  Missing Fields (CRÍTICO - debe tener MÚLTIPLES):" -ForegroundColor Yellow
foreach ($field in $missing_fields) {
    Write-Host "    • $field" -ForegroundColor White
}
Write-Host ""

# Verificar si se hace multiple questions (mejora esperada)
$field_count = $missing_fields.Count
if ($field_count -ge 2) {
    Write-Host "✅ MEJORA DETECTADA: $field_count preguntas estratégicas" -ForegroundColor Green
} else {
    Write-Host "❌ SIN MEJORA: Solo $field_count pregunta (esperado: 2+)" -ForegroundColor Red
}
Write-Host ""

# TURNO 2: Responder con motor
Write-Host "📍 TURNO 2: Responder con motor" -ForegroundColor Yellow
$turno2_query = "Es con motor diésel, cilindrada de 5900 cc. Es importado nuevo."
...
Write-Host "✓ TURNO 2 guardado en: $turno2_file" -ForegroundColor Green
...
# TURNO 3: Responder con capacidad
Write-Host "📍 TURNO 3: Responder con capacidad" -ForegroundColor Yellow
$turno3_query = "Es para 50 personas, de transporte público"
...
Write-Host "✓ TURNO 3 guardado en: $turno3_file" -ForegroundColor Green
...
Write-Host "  Código Final: $turno3_code (confianza: $turno3_confidence)" -ForegroundColor White
Write-Host "  Missing Fields:" -ForegroundColor Yellow
...
# Capturar logs de Docker
Write-Host "📍 Capturando Docker logs..." -ForegroundColor Yellow
...
Write-Host "✓ Docker logs guardados" -ForegroundColor Green

# Capturar Redis conversation
Write-Host "📍 Capturando Redis conversation..." -ForegroundColor Yellow
$redis_key = "conversation:$session_id"
...
Write-Host "✓ Redis conversation guardada" -ForegroundColor Green
...
Write-Host ""
Write-Host "📂 Todos los archivos en: $output_dir" -ForegroundColor Green
