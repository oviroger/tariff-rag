#!/usr/bin/env python3
"""
Reingestar datos de 2025 con extracción de chapter/heading/subheading.
...
CHECKPOINT_FILE = Path("scripts/reingest_2025_checkpoint.json")
...
def main():
    # Directorio con JSONs de 2025 (solo partes, sin referencias)
    data_dir = Path("data/afr_2025_partes_only")
    ...
    if not json_files:
        logger.error(f"No se encontraron archivos 'Arancel_Boliviano*.json' en {data_dir}")
        sys.exit(1)
    
    logger.info(f"Encontrados {len(json_files)} archivos Arancel")
    ...
    index_name = "tariff_fragments_2025"
    ...
    for json_file in files_to_process:
        fragments = process_afr_json(json_file, year=2025)
