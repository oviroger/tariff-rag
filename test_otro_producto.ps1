#!/usr/bin/env pwsh
# Test script para verificar mejora con OTRA MERCANCÍA (Electrodoméstico - Lavadora)

$API_URL = "http://localhost:8000/classify"
$timestamp = Get-Date -Format "yyyyMMdd_HHmmss"
$output_dir = "d:\MAESTRIA - copia\tariff-rag\test_results_lavadora_$timestamp"

New-Item -ItemType Directory -Path $output_dir -Force | Out-Null

Write-Host "=== PRUEBA CON OTRA MERCANCÍA: LAVADORA ===" -ForegroundColor Green
Write-Host "Timestamp: $timestamp" -ForegroundColor Cyan
Write-Host "Output Directory: $output_dir" -ForegroundColor Cyan
Write-Host ""

# TURNO 1: Query inicial sobre lavadora
Write-Host "📍 TURNO 1: Query inicial sobre lavadora" -ForegroundColor Yellow
$turno1_query = "Estoy importando una lavadora automática"
...
if ($field_count -ge 2) {
    Write-Host "✅ Múltiples preguntas generadas: $field_count" -ForegroundColor Green
} else {
    Write-Host "⚠️  Solo $field_count pregunta(s)" -ForegroundColor Yellow
}
Write-Host ""

# TURNO 2: Responder con especificaciones de la lavadora
Write-Host "📍 TURNO 2: Responder con especificaciones" -ForegroundColor Yellow
$turno2_query = "Es de carga frontal, capacidad de 8 kg"
...
# TURNO 3: Responder con más detalles
Write-Host "📍 TURNO 3: Responder con detalles adicionales" -ForegroundColor Yellow
$turno3_query = "Es de uso doméstico, voltaje 220V, es nueva"
...
Write-Host ""
Write-Host "=== RESUMEN FINAL - LAVADORA ===" -ForegroundColor Green
