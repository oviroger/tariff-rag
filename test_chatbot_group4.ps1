...
function Build-PartialAnswer($missingFields, $usedAnswers, $productType) {
    ...
    foreach ($field in $missingFields) {
        ...
        switch ($productType) {
            "paint" {
                if ($f -match "tipo|acrilica|esmalte|latex") { $answers += "Pintura acrílica"; continue }
                if ($f -match "base|agua|solvente|aceite") { $answers += "Base agua"; continue }
                if ($f -match "uso|interior|exterior|industrial") { $answers += "Uso interior"; continue }
                if ($f -match "envase|litros|galon") { $answers += "Envase 4 litros"; continue }
            }
            "medicine" {
                if ($f -match "tipo|analgesico|antibiotico|vitamina") { $answers += "Analgésico"; continue }
                if ($f -match "principio activo|composicion") { $answers += "Paracetamol"; continue }
                if ($f -match "presentacion|tableta|jarabe|inyectable") { $answers += "Tabletas"; continue }
                if ($f -match "dosis|concentracion|mg") { $answers += "500 mg"; continue }
            }
            "fertilizer" {
                if ($f -match "tipo|nitrogeno|fosforo|organico") { $answers += "Fertilizante NPK"; continue }
                if ($f -match "composicion|formula|concentracion") { $answers += "15-15-15"; continue }
                if ($f -match "presentacion|granulado|liquido") { $answers += "Granulado"; continue }
                if ($f -match "uso|agricola|jardineria") { $answers += "Uso agrícola"; continue }
            }
            "detergent" {
                if ($f -match "tipo|liquido|polvo|capsulas") { $answers += "Detergente líquido"; continue }
                if ($f -match "uso|ropa|vajilla|industrial") { $answers += "Para ropa"; continue }
                if ($f -match "biodegradable|fosfatos") { $answers += "Biodegradable"; continue }
                if ($f -match "envase|litros|kg") { $answers += "Envase 2 litros"; continue }
            }
            "glue" {
                if ($f -match "tipo|blanca|contacto|cianocrilato|epoxi") { $answers += "Cola blanca"; continue }
                if ($f -match "uso|madera|papel|ceramica|metal") { $answers += "Para madera"; continue }
                if ($f -match "base|agua|solvente") { $answers += "Base agua"; continue }
                if ($f -match "envase|gramos|ml") { $answers += "Envase 250 ml"; continue }
            }
        }
    }
...
# Escenarios: Químicos y farmacéuticos ambiguos
$scenarios = @(
    @{ name = "Pintura"; query = "Pintura"; type = "paint" },
    @{ name = "Medicamento"; query = "Medicamento"; type = "medicine" },
    @{ name = "Fertilizante"; query = "Fertilizante"; type = "fertilizer" },
    @{ name = "Detergente"; query = "Detergente"; type = "detergent" },
    @{ name = "Pegamento"; query = "Pegamento"; type = "glue" }
)

$fullOutput = "=== TEST CHATBOT CON RESPUESTAS PARCIALES - GRUPO 4: QUÍMICOS Y FARMACÉUTICOS ===`n"
